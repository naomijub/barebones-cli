...
use barebones::{
    ...
    tracer::{TelemetryConfig, init_telemetry},
    updater::Updater,
};
use clap::Parser;
...
fn main() -> anyhow::Result<()> {
    ...
    let logger = initialize_logger(&command.logging, settings.is_machine);
    ...
    let shutdown = init_telemetry(config, logger).map_err(|err| anyhow!("{}", err))?;
    ...
    let meter = opentelemetry::global::meter("plugins-loaded");
    let request_counter = meter
        .u64_counter("app.loaded_plugins")
        .with_description("total loaded plugins")
        .with_unit("count")
        .build();

    request_counter.add(manager.count() as u64, &[]);
    ...
    shutdown.shutdown().map_err(|err| anyhow!("{}", err))?;
    Ok(())
}
